...
class CorsaSwitch(devices.GenericSwitchDevice):
    ...
    def _format_commands(self, commands, **kwargs):
        ...
        if not all(kwargs.values()):
            raise exc.GenericSwitchNetmikoMethodError(
                cmds=commands, args=kwargs
            )
        try:
    ...
    def add_network(
        ...
        isVFCHost = True
        if (
            not "VFCHost" in self.config
            or not self.config["VFCHost"] == "True"
        ):
            LOG.info(
                "PRUTH: Skipping VFC Creation: isVFCHost " + str(isVFCHost)
            )
            isVFCHost = False
            return
        ...
        try:
            if self.config.has_key("provisioningVLAN"):
                if str(segmentation_id) == self.config["provisioningVLAN"]:
                    LOG.info(
                        "Creating provisioning network on VLAN "
        ...
        try:
            with ngs_lock.PoolLock(self.locker, **self.lock_kwargs):
                ...
                corsavfc.bridge_add_controller(
                    headers,
                    url_switch,
                    br_id=c_br,
                    cont_id=cont_id,
                ...
                for uplink in c_uplink_ports.split(","):
                    ...
                    corsavfc.bridge_attach_tunnel_ctag_vlan(
                        headers,
                        url_switch,
                        br_id=c_br,
                        ofport=ofport,
            ...
            LOG.error(
                "Failed add network. attempting to cleanup bridge: "
                + str(e)
                + ", "
    ...
    def add_network_to_existing_vfc(
        self, segmentation_id, network_id, bridge, vfc_name, of_controller=None
    ):
        token = self.config["token"]
        headers = {"Authorization": token}
        ...
        try:
            with ngs_lock.PoolLock(self.locker, **self.lock_kwargs):
                ...
                c_br_descr = corsavfc.get_bridge_descr(
                    headers, url_switch, c_br
                )
                c_br_descr = c_br_descr + "-" + str(segmentation_id)
                corsavfc.bridge_modify_descr(
                    headers, url_switch, c_br, c_br_descr
                )
    ...
    def find_named_vfc(self, vfc_name):
        ...
        try:
            ...
            LOG.error(
                "failed get bridge by vfc name: " + traceback.format_exc()
            )
            raise e
    ...
    def get_ofcontroller_ip_address(self, bridge):
        ...
        try:
            ...
            LOG.error(
                "failed get ofcontroller ip address: " + traceback.format_exc()
            )
            raise e
        # OF Controllers have the name as CONTbr<N>
        ofcontroller_name = "CONT" + str(bridge)
        ofcontroller_href = ofcontroller.json()["links"][ofcontroller_name][
            "href"
        ]
        ofcontroller_details = corsavfc.get_info(
            headers, url_switch, ofcontroller_details
        )
    ...
    def del_network(self, segmentation_id, project_id):
        ...
        if (
            ...
            LOG.info(
                "PRUTH: Skipping VFC Deletion: isVFCHost " + str(isVFCHost)
            )
            isVFCHost = False
            return
        ...
        try:
            with ngs_lock.PoolLock(self.locker, **self.lock_kwargs):
                ...
                if not bridge == None:
                    LOG.info(
                        "PRUTH: del_network - network in ByocVfc: "
                        + str(bridge)
                    )
                    ...
                    if len(VLAN_tags) > 1:
                        br_descr = br_descr.replace(
                            "-" + str(segmentation_id), ""
                        )
                        ofport = segmentation_id
                        corsavfc.bridge_modify_descr(
                            headers, url_switch, bridge, br_descr
                        )
                        corsavfc.bridge_detach_tunnel(
                            headers, url_switch, bridge, ofport
                        )
                    else:
                    ...
                    sharedNonByocVfc_tunnel = (
                        corsavfc.get_tunnel_by_bridge_and_ofport(
    ...
    def plug_port_to_network(
        self, port, segmentation_id, sdn_node_id, vfc_host
    ):
        ...
        LOG.info(
            "PRUTH: plug_port_to_network - switch: "
            + str(self)
            + ",  switch: "
            + self.config["name"]
        )
        LOG.info(
            "PRUTH: plug_port_to_network - self.config: " + str(self.config)
        )
        LOG.info(
            "PRUTH: plug_port_to_network - Binding port "
            + str(port)
            + " to network "
            + str(segmentation_id)
        )
            ...
            with ngs_lock.PoolLock(self.locker, **self.lock_kwargs):
                ...
                if not byocVFC == None:
                    ...
                    if str(uplink_ofport) == str(sharedNonByocVfc_tunnel):
                        ...
                        LOG.info(
                            "PRUTH: plug_port_to_network - Network missing in sharedNonByocVFC"
                        )

                LOG.info(
                    "PRUTH: plug_port_to_network - br_id : "
                    + str(br_id)
                    + " for network "
                    + str(segmentation_id)
                )
                ...
                if br_id == sharedNonByocVFC:
                    ...
                    LOG.info(
                        "PRUTH: plug_port_to_network - sharedNonByocVFC: "
                        + str(br_id)
                        + " ofport: "
                        + str(ofport)
                    )
                    ...
                    LOG.info(
                        "PRUTH: plug_port_to_network - ByocVFC: "
                        + str(br_id)
                        + " ofport: "
                        + str(ofport)
                    )
                ...
                if "name" in self.config:
                    ...
                    LOG.info(
                        "PRUTH: plug_port_to_network - dst_switch_name "
                        + str(dst_switch_name)
                    )
                ...
                if not vfc_host == self:
                    ...
                    vfc_host.__bind_ctag_tunnel(
                        segmentation_id, node_vlan, dst_switch_name, ofport
                    )

                else:
                    # Step 2: config VFCHost (i.e. local host) with passthrough tunnel
                    self.__bind_passthrough_tunnel(
                        port, segmentation_id, ofport
                    )

        except Exception as e:
            LOG.error(
                "failed plug_port_to_network : " + traceback.format_exc()
            )
            raise e

    def plug_port_to_network_haswellsdn(
        self, port, segmentation_id, sdn_node_id, vfc_host
    ):
        ...
        LOG.info(
            "PRUTH: plug_port_to_network_haswellsdn - switch  : "
            + str(self)
            + ",  switch: "
            + self.config["name"]
        )
        LOG.info(
            "PRUTH: plug_port_to_network_haswellsdn - vfchost : "
            + str(vfc_host)
        )
        LOG.info(
            "PRUTH: plug_port_to_network_haswellsdn - self.config: "
            + str(self.config)
        )
        LOG.info(
            "PRUTH: plug_port_to_network_haswellsdn - Binding port "
            + str(port)
            + " to network "
            + str(segmentation_id)
        )
        ...
        try:
            with ngs_lock.PoolLock(self.locker, **self.lock_kwargs):
                ...
                LOG.info(
                    "PRUTH: plug_port_to_network_haswellsdn - br_id : "
                    + str(br_id)
                    + " for network "
                    + str(segmentation_id)
                )
                LOG.info(
                    "PRUTH: plug_port_to_network_haswellsdn - byocVFC : "
                    + str(byocVFC)
                )
                ...
                if "name" in self.config:
                    ...
                    LOG.info(
                        "PRUTH: plug_port_to_network_haswellsdn - dst_switch_name "
                        + str(dst_switch_name)
                    )
    ...
    def __bind_passthrough_tunnel(self, port, segmentation_id, ofport):
        ...
        corsavfc.port_modify_tunnel_mode(
            headers, url_switch, port_num, "passthrough"
        )
        ...
        LOG.info(
            "PRUTH: __bind_passthrough_tunnel - br_id: "
            + str(br_id)
            + " segmentation_id: "
            + str(segmentation_id)
            + " port_num: "
        ...
        corsavfc.bridge_attach_tunnel_passthrough(
            headers,
            url_switch,
            br_id,
            port_num,
    ...
    def __bind_ctag_tunnel(
        self, segmentation_id, node_vlan, dst_switch_name, ofport=None
    ):
        LOG.info(
            "PRUTH: __bind_ctag_tunnel - adding to vfc_host: __bind_ctag_tunnel"
        )
        LOG.info(
            "PRUTH: __bind_ctag_tunnel - switch: "
            + self.config["name"]
            + ", VFCHost: "
            + str(segmentation_id)
            + ", "
        ...
        LOG.info(
            "PRUTH: __bind_ctag_tunnel - About to dst_port: " + str(dst_port)
        )
        byocVFC = corsavfc.get_bridge_by_segmentation_id(
    ...
    def delete_port(
        self, port, segmentation_id, sdn_node_id, vfc_host, ofport=None
    ):
        ...
        if not byocVFC == None:
            ...
            br_id = sharedNonByocVFC
        LOG.info(
            "PRUTH: delete_port - br_id on VFCHost: "
            + str(br_id)
            + " for network "
            + str(segmentation_id)
        )
        ...
        if ofport == None:
            ...
            if br_id == sharedNonByocVFC:
                ...
                if not vfc_host == self:
                    ...
                    ofport = int(str(p_num) + v)
                LOG.info(
                    "PRUTH: delete_port - sharedNonByocVFC: "
                    + str(br_id)
                    + " ofport: "
                    + str(ofport)
                )
                ...
                ofport = str(int(sdn_node_id) + 10000)
                LOG.info(
                    "PRUTH: delete_port - ByocVFC: "
                    + str(br_id)
                    + " ofport: "
                    + str(ofport)
                )
        if not vfc_host == self:
            vfc_host.delete_port(
                port, segmentation_id, sdn_node_id, vfc_host, ofport=ofport
            )
        else:
    ...
    def __delete_ofport(self, ofport, segmentation_id):
        ...
        try:
            with ngs_lock.PoolLock(self.locker, **self.lock_kwargs):
                ...
                corsavfc.bridge_detach_tunnel(
                    headers, url_switch, br_id, ofport
                )
